...
class MDRegexSplitter(BaseSplitter):
    regex_pattern = r"""# Negative lookbehind for bold, emphasized, and strikethrough
(?<!(?:\*\*|\*|~~))
...
(?<=[!?])(?=\s|$)
# Negative lookahead for bold, emphasized, and strikethrough
(?!(?:\*\*|\*|~~))"""

    def __call__(self, doc: str, delimiters: List[Union[str, regex.Pattern]] = []) -> List[str]:
        # Remove single newlines, preserving double or more newlines
        doc = regex.sub(r"(?<!\n)\n(?!\n)", " ", doc)
        ...
        for delimiter in delimiters:
            ...
            for sentence in sentences:
                if isinstance(delimiter, regex.Pattern):
                    sub_sentences = regex.split(self.regex_pattern, sentence, flags=regex.VERBOSE)
